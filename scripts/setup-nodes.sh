#!/usr/bin/env bash
# Spin up six independent anet daemons on this laptop and seed cross-node
# credit ledgers so priced calls don't hit 402.
#
# Layout (matches CLAUDE.md / SPEC.md):
...
#
# Daemons 2-6 bootstrap off daemon-1 to form one mesh.

set -euo pipefail
ANET="${ANET:-anet}"

API=(14101 14102 14103 14104 14105 14106)
P2P=(14201 14202 14203 14204 14205 14206)
HOMES=(/tmp/anet-ci-u1 /tmp/anet-ci-u2 /tmp/anet-ci-u3 /tmp/anet-ci-u4 /tmp/anet-ci-u5 /tmp/anet-ci-u6)
...
Next: bash scripts/run-all.sh    # start the 6 agents
EOF
}

cmd_stop() {
  for p in "${API[@]}" "${P2P[@]}" 7401 7402 7403 7404 7405 7406; do
    lsof -ti tcp:"$p" 2>/dev/null | xargs -r kill -9 2>/dev/null || true
  done
