#!/usr/bin/env bash
# Start all six agent backends as background processes, each pinned to its
# own daemon HOME so register.py reads the correct api_token. Logs go to
# /tmp/anet-ci-u<N>/agent.log; PIDs to /tmp/anet-ci-pids.
...
sleep 3
green "all six agents up; PIDs in $PIDFILE"
